...
class bigFileEditor(QDialog):
  def __init__(self, parent=None):
    ...
    #
    """etiNbLi= QLabel("Nb de lignes :")
    hLayout1.addWidget(etiNbLi)
    self.lNbRows= QComboBox(self)
    self.lNbRows.setEditable(True)
    self.lNbRows.setObjectName("lNbRows")
    self.lNbRows.setToolTip("Choisir le nombre de lignes à afficher")
    self.lNbRows.addItem('20',20)
    self.lNbRows.addItem('50',50)
    self.lNbRows.addItem('100',100)
    self.lNbRows.addItem('200',200)
    self.lNbRows.addItem('500',500)
    self.lNbRows.addItem('1000',1000)
    self.lNbRows.currentIndexChanged[int].connect(self.lNbRowsChoice) #self.lNbRows.activated[int].connect(self.lNbRowsChoice)
    self.lNbRows.editTextChanged.connect(self.lNbRowsText)
    hLayout1.addWidget(self.lNbRows)
    def lNbRowsChoice(self, index):
      print( self.lNbRows.itemData(index), self.lNbRows.itemData(index).__class__.__name__ )
    def lNbRowsText(self, text): 
      text= self.lNbRows.currentText()
      #index = self.lNbRows.findText("%s" % (text))
      print( "lNbRows.currentText() : "+ text ) #"""
    ...
    self.sbNbRows.setToolTip(self.tr("Minimum: 1 - Maximum: 10000"))
